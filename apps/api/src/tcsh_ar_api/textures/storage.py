...
class SupabaseStorage:
    ...
    async def create_upload_url(self, filename: str) -> SignedUpload:
        ...
        if not signed_url or not token:
            # Leave a breadcrumb of both the keys present and whether each
            # expected value was truthy, so log triage can tell "key
            # renamed / missing" apart from "key present but empty string"
            # without having to reproduce the SDK response.
            logger.warning(
